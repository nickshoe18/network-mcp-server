...
This is the canonical path; it replaces the retired legacy JSON declarative
engine (``engine.py`` / ``loader.py`` / ``lint.py`` / ``targets/*.json``) for
every AOS8→Central translation kind, plus WLAN (Mist/Central/AOS8).
"""
...
async def resolve_central_wlan_scopes(conn: Any) -> dict[str, Any]:
    ...
    # NOTE (local adaptation): unlike upstream, local's build_scope_tree is a
    # plain synchronous function (it calls the sync retry_central_command, not
    # an async client) — see src/hpe_networking_mcp/platforms/central/utils.py.
    # No `await` here, or this raises "object dict can't be used in 'await'
    # expression". resolve_central_wlan_scopes itself stays `async def` so its
    # call sites (translate_wlan.py's `await orchestrator.resolve_central_wlan_scopes(conn)`)
    # are unaffected.
