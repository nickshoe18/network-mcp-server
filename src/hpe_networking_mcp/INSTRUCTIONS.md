...
- **`MCP_TOOL_MODE=dynamic`** (opt-in since v3.0.0.0; was the v2.x default) — 28 tools visible:
    - **4 cross-platform aggregator tools**: `health`, `site_health_check`, `site_rf_check`, `manage_wlan_profile`
    - **4 cross-platform translation-bridge tools**: `translate_wlan_preview`, `translate_wlan_apply`, `translate_config_preview`, `translate_config_apply`
...
- `translate_wlan_preview(source_platform, target_platform, ssid, ...)` / `translate_wlan_apply(...)` — translate a WLAN between platforms (mist/central/aos8 → central/mist) via the canonical translation engine. Preview is read-only (secrets redacted); apply is target-write-gated (`ENABLE_*_WRITE_TOOLS`) + confirmed. **Registered in every mode (incl. code).**
- `translate_config_preview(source_platform="aos8", target_platform="central", kind, source_record, scope_id, ...)` / `translate_config_apply(...)` — same engine for the 12 non-WLAN AOS8→Central config kinds (vlan_id / named_vlan / net_group / role / policy / the AAA chain / gateway_cluster). Preview is read-only; apply is gated by `ENABLE_CENTRAL_WRITE_TOOLS` + confirmation, idempotent (ensure-or-create), and blocks `kind="auth_server"` with a 403 until AOS 8 secret tokenization ships. **Registered in every mode (incl. code).**

> **Code mode caveat:** of the six cross-platform tools above, `health`, `translate_wlan_preview` / `translate_wlan_apply`, and `translate_config_preview` / `translate_config_apply` are registered in `code` mode (reachable from `execute()` via `call_tool`) — `site_health_check`, `site_rf_check`, and `manage_wlan_profile` are deliberately NOT (code mode's premise is that the AI composes per-platform tools itself). In code mode:
...
> - **Site health** → compose the per-platform tools directly (use `search` / `tags` to find them).
> - **WLAN sync/migration** → call `translate_wlan_preview` / `translate_wlan_apply` directly (or the `aos-migration` skill for AOS8→Central).
> - **AOS8→Central config migration** (vlan / role / policy / net_group / AAA chain / gateway_cluster) → call `translate_config_preview` / `translate_config_apply` directly (or the `aos-migration` skill).
...
- **Config translation (AOS 8 → Central)**: `translate_config_preview` / `translate_config_apply` (**registered in every mode, incl. code** — reachable from `execute()` via `call_tool`). Supersedes the retired preview-only translation-preview bridge tool — these wrap the canonical translation engine (`hpe_networking_mcp.translations`: `canonical/` + `readers/` + `writers/` + `orchestrator.py`) and now cover both preview AND apply.
  - `translate_config_preview(source_platform="aos8", target_platform="central", kind, source_record, scope_id, device_functions?, extra_ctx?)` — read-only; returns `{supported, canonical, calls, unresolved, preview}` with secrets redacted. `kind` is one of `vlan_id` / `named_vlan` / `net_group` / `role` / `policy` / `auth_server` / `server_group` / `dot1x_auth` / `mac_auth` / `captive_portal` / `aaa_profile` / `gateway_cluster`. Use for "show me what migrating these AOS 8 policies / roles / VLANs would produce in Central" workflows. The `aos-migration` skill drives it.
  - `extra_ctx` supplies kind-specific reader inputs: `role_records` (policy — full AOS 8 role list, for role-attribution reverse-index), `coa_servers` (auth_server — for AUTH_AND_COA correlation), `cluster_strategy` (gateway_cluster — `ha_only` | `intent_site` | `intent_manual`), `alias_name` (named_vlan override).
  - `translate_config_apply(...)` — executes the previewed calls against Central; gated by `ENABLE_CENTRAL_WRITE_TOOLS` + the universal elicitation confirmation; idempotent (ensure-or-create — skips a create whose target already exists). **Blocks `kind="auth_server"` (and any body carrying a cleartext secret) with a 403** until AOS 8 secret tokenization ships — preview it and create the auth-server manually for now.
  - A `None`/unresolved `scope_id` (or an unresolved gateway-cluster binding) is surfaced in `unresolved` and blocks `apply` — resolve the scope/cluster first.
