...
def _central_command(conn: Any) -> orchestrator.ClientCall:
    ...
    Local adaptation: upstream calls ``conn.command`` directly as an async
    positional callable. Locally, ``NewCentralBase.command`` (pycentral) is
    SYNCHRONOUS and keyword-only (``api_method=``/``api_path=``/``api_params=``/
    ``api_data=``) — see platforms/central/utils.py:retry_central_command, the
    same helper every other local Central write tool uses. This wrapper is an
    ``async def`` (so ``await command(...)`` call sites in orchestrator.py keep
...
async def _apply_impl(
    ...
    *,
    ...
    results = await orchestrator.execute(_central_command(conn), plan)
