"""Classic Central tool registry — module-level FastMCP holder + shared-registry shim.

Tool modules under ``platforms/classic_central/tools/`` decorate their
...
3. Populates the shared ``REGISTRIES["classic_central"]`` dict so the
...
# Set by ``platforms.classic_central.register_tools()`` before tool modules are imported.
mcp: FastMCP = None  # type: ignore[assignment]

_PLATFORM = "classic_central"
