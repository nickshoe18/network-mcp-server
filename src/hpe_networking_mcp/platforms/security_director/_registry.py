"""Security Director tool registry — module-level FastMCP holder + shared-registry shim.

Tool modules under ``platforms/security_director/tools/`` decorate their
functions with ``@tool(...)`` (imported from here) instead of directly with
``@mcp.tool(...)``. The shim:
...
3. Populates the shared ``REGISTRIES["security_director"]`` dict so the
   meta-tools can dispatch by name at runtime.
...
# Set by ``platforms.security_director.register_tools()`` before tool modules are imported.
mcp: FastMCP = None  # type: ignore[assignment]

_PLATFORM = "security_director"
