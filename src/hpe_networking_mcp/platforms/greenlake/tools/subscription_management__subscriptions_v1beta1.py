"""GreenLake Subscription Management tools -- ported from upstream nowireless4u/hpe-networking-mcp.

Ported from ``platforms/greenlake/tools/subscription_management__subscriptions_v1beta1.py`` in the upstream repo (generated
there from a vendored OpenAPI spec). Upstream's generator, ``Capability``
classification enum, and ``AsyncTokenManager`` auth were NOT ported -- this
file uses local's inline dict-annotation style and the ``greenlake_request()``
shim added to ``client.py`` for the actual HTTP transport.

Upstream service: ``subscription-management``   tag: ``subscriptions_v1beta1``   operations: 5
"""

from __future__ import annotations

from typing import Annotated, Any

from fastmcp import Context
from pydantic import Field

from hpe_networking_mcp.platforms._common.url import path_seg
from hpe_networking_mcp.platforms.greenlake._registry import tool
from hpe_networking_mcp.platforms.greenlake.client import greenlake_request


@tool(
    name="greenlake_get_subscriptions_v1beta1_async_operations_id",
    description="GET /subscriptions/v1beta1/async-operations/{id}\n\ngetSubscriptionsAsyncOperationResource\n\nGet progress or status of async operations in subscriptions",
    tags={"greenlake", "subscription_management"},
    annotations={
        "title": "Get progress or status of async operations in subscriptions",
        "readOnlyHint": True,
        "destructiveHint": False,
        "idempotentHint": True,
        "openWorldHint": True,
    },
)
async def greenlake_get_subscriptions_v1beta1_async_operations_id(
    ctx: Context,
    id: Annotated[str, Field(description="The unique identifier returned by an asynchronous API operation.")],
) -> Any:
    path = f"/subscriptions/v1beta1/async-operations/{path_seg(id)}"
    return await greenlake_request(
        ctx,
        "GET",
        path,
    )


@tool(
    name="greenlake_get_subscriptions_v1beta1_subscriptions",
    description="GET /subscriptions/v1beta1/subscriptions\n\ngetSubscriptionsBeta\n\nGet subscriptions of a workspace",
    tags={"greenlake", "subscription_management"},
    annotations={
        "title": "Get subscriptions of a workspace",
        "readOnlyHint": True,
        "destructiveHint": False,
        "idempotentHint": True,
        "openWorldHint": True,
    },
)
async def greenlake_get_subscriptions_v1beta1_subscriptions(
    ctx: Context,
    filter: Annotated[
        str | None,
        Field(
            default=None,
            description="Filter expressions consisting of simple comparison operations joined  by logical operators.<br> | CLASS                |   EXAMPLES                                         | |----------------------|----------------------------------------------------| | Types                | integer, decimal, timestamp, string, boolean, null | | Comparison           | eq, ne, gt, ge, lt, le, in                         | | Logical Expressions  | and, or, not                                       |  The following is a non-exhaustive list of possible filtering options.",
        ),
    ] = None,
    filter_tags: Annotated[
        str | None,
        Field(
            default=None,
            description="Filter expressions consisting of simple comparison operations joined by logical operators to be applied on the assigned tags or their values.<br> | CLASS               |   EXAMPLES      | |---------------------|-----------------| | Types               | string          | | Comparison          | eq, ne          | | Logical Expressions | and, or         |",
        ),
    ] = None,
    sort: Annotated[
        str | None,
        Field(
            default=None,
            description="A comma separated list of sort expressions. A sort expression is a  property name optionally followed by a direction indicator `asc` or  `desc`. The default is ascending order.",
        ),
    ] = None,
    select: Annotated[
        list[str] | None,
        Field(
            default=None,
            description="A comma separated list of select properties to display in the response.  The default is that all properties are returned.",
        ),
    ] = None,
    limit: Annotated[
        int | None,
        Field(default=None, description="Specifies the number of results to be returned. The default value  is 50."),
    ] = None,
    offset: Annotated[
        int | None,
        Field(
            default=None,
            description="Specifies the zero-based resource offset to start the response from. The default value is 0.",
        ),
    ] = None,
) -> Any:
    query_params: dict[str, Any] = {}
    if filter is not None:
        query_params["filter"] = filter
    if filter_tags is not None:
        query_params["filter-tags"] = filter_tags
    if sort is not None:
        query_params["sort"] = sort
    if select is not None:
        query_params["select"] = select
    if limit is not None:
        query_params["limit"] = limit
    if offset is not None:
        query_params["offset"] = offset
    return await greenlake_request(
        ctx,
        "GET",
        "/subscriptions/v1beta1/subscriptions",
        query_params=query_params or None,
    )


@tool(
    name="greenlake_get_subscriptions_v1beta1_subscriptions_id",
    description="GET /subscriptions/v1beta1/subscriptions/{id}\n\ngetSubscriptionDetailsByID\n\nGet information on a subscription",
    tags={"greenlake", "subscription_management"},
    annotations={
        "title": "Get information on a subscription",
        "readOnlyHint": True,
        "destructiveHint": False,
        "idempotentHint": True,
        "openWorldHint": True,
    },
)
async def greenlake_get_subscriptions_v1beta1_subscriptions_id(
    ctx: Context,
    id: Annotated[str, Field(description="The unique identifier of the subscription.")],
) -> Any:
    path = f"/subscriptions/v1beta1/subscriptions/{path_seg(id)}"
    return await greenlake_request(
        ctx,
        "GET",
        path,
    )


@tool(
    name="greenlake_patch_subscriptions_v1beta1_subscriptions",
    description="PATCH /subscriptions/v1beta1/subscriptions\n\npatchSubscriptions\n\nUpdate subscriptions",
    tags={"greenlake", "greenlake_write", "requires_confirmation", "subscription_management"},
    annotations={
        "title": "Update subscriptions",
        "readOnlyHint": False,
        "destructiveHint": False,
        "idempotentHint": False,
        "openWorldHint": True,
    },
)
async def greenlake_patch_subscriptions_v1beta1_subscriptions(
    ctx: Context,
    id: Annotated[list[str], Field(description="Subscription resourceIDs.")],
    body: Annotated[dict[str, Any], Field(description="Request body (required)")],
) -> Any:
    query_params: dict[str, Any] = {}
    if id is not None:
        query_params["id"] = id
    return await greenlake_request(
        ctx,
        "PATCH",
        "/subscriptions/v1beta1/subscriptions",
        query_params=query_params or None,
        body=body,
    )


@tool(
    name="greenlake_post_subscriptions_v1beta1_subscriptions",
    description="POST /subscriptions/v1beta1/subscriptions\n\npostSubscriptions\n\nAdd subscriptions",
    tags={"greenlake", "greenlake_write", "requires_confirmation", "subscription_management"},
    annotations={
        "title": "Add subscriptions",
        "readOnlyHint": False,
        "destructiveHint": False,
        "idempotentHint": False,
        "openWorldHint": True,
    },
)
async def greenlake_post_subscriptions_v1beta1_subscriptions(
    ctx: Context,
    body: Annotated[dict[str, Any], Field(description="Request body (required)")],
) -> Any:
    return await greenlake_request(
        ctx,
        "POST",
        "/subscriptions/v1beta1/subscriptions",
        body=body,
    )
