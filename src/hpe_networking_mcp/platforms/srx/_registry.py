"""SRX tool registry — module-level FastMCP holder + shared-registry shim.

Tool modules under ``platforms/srx/tools/`` decorate their functions with
``@tool(...)`` (imported from here) instead of directly with
``@mcp.tool(...)``. The shim:
...
3. Populates the shared ``REGISTRIES["srx"]`` dict so the meta-tools can
...
# Set by ``platforms.srx.register_tools()`` before tool modules are imported.
...
def _derive_category(func: Callable[..., Any]) -> str:
    """Short module name as the registry category (e.g. ``device``)."""
