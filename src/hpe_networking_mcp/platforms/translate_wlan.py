...
async def _fetch_central_source(ctx: Context, ssid: str) -> tuple[dict, dict]:
    ...
    # Local adaptation: upstream calls `await conn.command("GET", path)`
    # positionally (an async client). Locally, NewCentralBase.command
    # (pycentral) is synchronous and keyword-only, so these one-off GETs go
    # through retry_central_command directly (no `await` — it's a plain
    # function) — same helper every other local Central read/write tool uses.
    # These helper functions stay `async def` so `await _fetch_central_source(...)`
    # at the call site in `_build_plan` is unaffected.
    async def _list(path: str, key: str) -> list[dict]:
        m = retry_central_command(
            central_conn=conn, api_method="GET", api_path=f"network-config/v1alpha1/{path_seg(path)}"
        )["msg"]
        return m.get(key, []) if isinstance(m, dict) else []

    w = retry_central_command(
        central_conn=conn, api_method="GET", api_path=f"network-config/v1alpha1/wlan-ssids/{path_seg(ssid)}"
    )["msg"]
...
# --------------------------------------------------------------------------- #
def _central_command(conn: Any) -> orchestrator.ClientCall:
    """Adapt local's synchronous ``retry_central_command`` to the orchestrator's
    async ``(method, path, api_params=, api_data=) -> {code, msg}`` ``ClientCall``.

    Local adaptation: upstream passes ``conn.command`` straight through as the
    async command callable. Locally, ``NewCentralBase.command`` (pycentral) is
    SYNCHRONOUS and keyword-only — see platforms/central/utils.py:retry_central_command,
    the same helper every other local Central write tool uses. This wrapper is
    an ``async def`` (so ``await command(...)`` in orchestrator.py keeps
    working) whose body calls the sync helper directly, with no ``await``.
    """

    async def command(method: str, path: str, api_params: dict | None = None, api_data: Any = None) -> dict:
        return retry_central_command(
            central_conn=conn, api_method=method, api_path=path, api_params=api_params, api_data=api_data
        )
